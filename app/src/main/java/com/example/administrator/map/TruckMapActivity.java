...
import com.example.administrator.models.SpinnerData;
...
public class TruckMapActivity extends Activity {
...
//    }
                    ...
                    public void onResponse(String s) {
                        if (s == null && s.equals("")) {
                            ...
                            try {
                                ...
                                ArrayAdapter<SpinnerData> adapter = new ArrayAdapter<SpinnerData>(TruckMapActivity.this, android.R.layout.simple_spinner_item, listSpinner);
    ...
    private void GetEmpByDept(String dept) {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String s) {
                        if (s == null && s.equals("")) {
                            Toast.makeText(TruckMapActivity.this, "未找到数据", Toast.LENGTH_SHORT).show();
                            return;
                        } else if (s != null) {
                            try {
                                JSONArray array = new JSONArray(s);
    ...
    private void GetName(String dept) {
        final List<SpinnerData> listSpinner = new ArrayList<SpinnerData>();
        String url = this.getString(R.string.requestUrl) + "GetTruckName?dept=" + dept;
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String s) {
                            if (s.equals("")) {
                                ...
                                Toast.makeText(TruckMapActivity.this,"未找到车辆数据",Toast.LENGTH_SHORT).show();
                                return;
                            } else if (s != null) {
                                try {
                                    JSONArray array = new JSONArray(s);
                                    for (int i = 0; i < array.length(); i++) {
                                        JSONObject jsonObject = array.getJSONObject(i);
                                        SpinnerData data = new SpinnerData(jsonObject.getString("GPS_SIMID"), jsonObject.getString("TRUCK_CODE"));
                                        listSpinner.add(data);
                                    }
                                    ArrayAdapter<SpinnerData> adapter = new ArrayAdapter<SpinnerData>(TruckMapActivity.this, android.R.layout.simple_spinner_item, listSpinner);
                                    adapter.setDropDownViewResource(android.R.layout.simple_spinner_item);
    ...
    private void GetCustomByName(String Name){
        mBaiduMap.clear();
        String url = this.getString(R.string.requestUrl) + "GetTruckCustomByName?Name=" + Name;
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String s) {
    ...
    private void GetEmpinformation(String Name){
        String url = this.getString(R.string.requestUrl) + "GetTruckinformation?Name=" + Name;
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String s) {
