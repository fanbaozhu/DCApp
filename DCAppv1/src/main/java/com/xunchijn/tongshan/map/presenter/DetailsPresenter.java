...
public class DetailsPresenter implements DetailsContract.Presenter {
	...
	public void getUserDetails(String id) {
		mEventService.GetUserinFormation(id).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new Observer<Response<Result<DetailsResult>>>() {
					@Override
					public void onSubscribe(Disposable d) {
						Log.d(TAG, "onSubscribe: ");
	...
	private void parseResult(Result<DetailsResult> result) {
		if (result.getCode() == 200) {
			...
			} else if (result.getData().getUserInfomation() != null) {
				mView.showUserDetails(result.getData().getUserInfomation());
			}
		} else {
